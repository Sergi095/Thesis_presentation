...
export const DRONE = Object.freeze({ mass: .027, inertia: [1.4e-5, 1.4e-5, 2.17e-5],
  arm: .0397, rotorOffset: .028, kf: 3.16e-10, km: 7.94e-12, radius: .06, height: .025, gravity: 9.8 });
...
export function euler(q) {
  const r=rotation(q);
  return [Math.atan2(r[2][1],r[2][2]), Math.asin(clip(-r[2][0],-1,1)), Math.atan2(r[1][0],r[0][0])];
}
export class FlightControl {
  ...
  compute(dt, position, quaternion, velocity, targetPosition, targetVelocity, targetYaw, targetYawRate=0) {
    ...
    const force=position.map((v,i)=>{
      ...
      this.integralPos[i]=clip(this.integralPos[i]+error*dt,i===2?-.15:-2,i===2?.15:2);
      return p[i]*error+.05*this.integralPos[i]+d[i]*(targetVelocity[i]-velocity[i])+(i===2?DRONE.mass*DRONE.gravity:0);
    });
    const thrust=(Math.sqrt(Math.max(0,dot(force,r.map(row=>row[2])))/(4*DRONE.kf))-4070.3)/.2685;
    const z=normalize(force), y=normalize(cross(z,[Math.cos(targetYaw),Math.sin(targetYaw),0])), x=cross(y,z);
    const desired=[0,1,2].map(i=>[x[i],y[i],z[i]]);
    const product=(a,b,i,j)=>a.reduce((s,row,k)=>s+row[i]*b[k][j],0);
    const skew=(i,j)=>product(desired,r,i,j)-product(r,desired,i,j);
...
export function motorWrench(rpm) {
  const forces=rpm.map(v=>DRONE.kf*v*v), torques=rpm.map(v=>DRONE.km*v*v), a=DRONE.rotorOffset;
  return { force:[0,0,forces.reduce((a,b)=>a+b,0)],
    torque:[a*(-forces[0]-forces[1]+forces[2]+forces[3]), a*(-forces[0]+forces[1]+forces[2]-forces[3]),-torques[0]+torques[1]-torques[2]+torques[3]] };
}
