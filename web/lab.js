...
export function createLaboratory(root) {
  ...
  <label>Sensing range R<input name="range" type="number" min="0.1" max="10" step="0.1" value="3" required></label>
  <label>Predator interaction<select name="model"><option value="dm">DM</option><option value="adm">ADM</option></select></label>
  <label>Capture distance<input name="capture" type="number" min="0.05" max="1" step="0.01" value="0.5" required></label>
  <p id="lab-scale-note">0.3× scale · Sensing: 0.90 m · Capture: 0.15 m</p>
  ...
  <details class="model-note"><summary>About the laboratory model</summary><p>The arena measures 4.40 × 7.90 × 2.20 metres. Crazyflie drones have a mass of 27 grams, rotor thrust and torque, inertia, and PID flight control. Bullet resolves gravity and collisions with other drones, the floor, walls and ceiling.</p><p>The thesis PyBullet swarm controller supplies horizontal flight commands at 20 Hz, with the thesis PyBullet 0.3× parameter-scaling rule: distances, noise and applicable gains are scaled once. Speed limits remain 0.15 m/s; physical dimensions and flight-control parameters retain their measured values. Settings above are before scaling; the effective sensing and capture distances appear below them.</p><p>Both swarms use the thesis boundary-repulsion formula within 0.50 m of a wall, enabled with a scaled gain of 0.30. There is no target. Flight control updates at 120 Hz and physics at 240 Hz. Predators fly at 0.60 m and prey at 0.50 m. Sensing and capture use horizontal distance, as in the planar PyBullet experiments; captured prey are removed.</p><p>Predators use positive and prey negative sensing modulation of their interaction spacing, with λ = 0.2. Both swarms start in equilibrium hexagonal formations with a nearest-agent gap of half the effective sensing range. This is a Bullet-based adaptation of the thesis laboratory profile. It is not a replay of PyBullet experiments or a claim of identical trajectories. Aerodynamic drag, ground effect and downwash are not modelled.</p></details>`;
  const $=id=>root.querySelector(`#${id}`), form=$('lab-controls');
  form.addEventListener('input',()=>{$('lab-scale-note').textContent=`0.3× scale · Sensing: ${(Number(form.elements.range.value)*.3).toFixed(2)} m · Capture: ${(Number(form.elements.capture.value)*.3).toFixed(2)} m`;});
