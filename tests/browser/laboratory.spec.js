...
test('second simulator advances physical drones, pauses, exports and retains the original simulator',async({page})=>{
  ...
  await expect(page.locator('#lab-scale-note')).toHaveText('0.3× scale · Sensing: 0.90 m · Capture: 0.15 m');
  await page.locator('#lab-controls input[name=range]').fill('5');
  await expect(page.locator('#lab-scale-note')).toContainText('Sensing: 1.50 m');
